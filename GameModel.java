...
public class GameModel implements Cloneable,Serializable {
   ...
   public void select(int i, int j){
    ...
    //does a deep copy of the current GameModel object 
    protected Object clone() throws CloneNotSupportedException{
     GameModel returnModel=(GameModel)super.clone();    //shallow copy of gameModel
     int[][] states=new int[sizeOfGame][sizeOfGame];    //deep copy of model
     for (int i =0;i<returnModel.model.length;i++){     //copying model
      for (int j=0;j<returnModel.model[i].length;j++){
        states[i][j]=returnModel.model[i][j];
      }
     }
     returnModel.model=states;
     //deep copy of currentDot
     returnModel.currentDot=new Point(returnModel.currentDot.getX(),returnModel.currentDot.getY());
     return returnModel;
    }
